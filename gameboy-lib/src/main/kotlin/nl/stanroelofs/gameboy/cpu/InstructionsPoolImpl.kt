...
import nl.stanroelofs.gameboy.cpu.instructions.alu.*
import nl.stanroelofs.gameboy.cpu.instructions.bit.*
...
import nl.stanroelofs.gameboy.cpu.instructions.jumps.*
import nl.stanroelofs.gameboy.cpu.instructions.loads.*
import nl.stanroelofs.gameboy.cpu.instructions.miscellaneous.*
...
import nl.stanroelofs.gameboy.cpu.instructions.rotates.*
import nl.stanroelofs.gameboy.cpu.instructions.shifts.*
...
class InstructionsPoolImpl(private val registers: Registers, private val mmu: Mmu) : InstructionsPool {
    private val instructions = Array(256) {
        when (it) {
            ...
            // DI
            0xF3 -> DI(registers, mmu)

            // EI
            0xFB -> EI(registers, mmu)
            ...
            else -> NOP(registers, mmu)
        }
    }
    private val extendedInstructions = Array(256) {
        when(it) {
            // Miscellaneous
            // SWAP r
            ...
            else -> throw IllegalStateException("Instruction does not exist")
        }
    }

    override fun getInstruction(opcode: Int): Instruction {
        val instruction = if (opcode == 0xCB) {
            val newOpcode = mmu.readByte(registers.PC)
            registers.incPC()
            extendedInstructions[newOpcode]
        } else {
            instructions[opcode]
        }
